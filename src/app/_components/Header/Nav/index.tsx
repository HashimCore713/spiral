...
import { Header as HeaderType, Category } from '../../../../payload/payload-types'
...
export const HeaderNav: React.FC<HeaderNavProps> = ({ header, categories, menuOpen }) => {
  ...
  const navItems = [
    ...
    !user && { label: 'Login', component: <Button el="link" href="/login" label="Login" appearance="primary" onClick={() => (window.location.href = '/login')} /> },
  ].filter(Boolean);
  ...
  return (
    ...
    >
      ...
      {categories.length < navItems.length && navItems.slice(categories.length).map((navItem, i) => (
